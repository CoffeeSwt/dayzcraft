    ...
    <main class="w-4/5 mx-auto pt-2">
      ...
      <a-space direction="vertical" fill>
        <template v-for="(item, index) in res">
          <a-card :title="`第${index + 1}个合成`" hoverable>
            <div class="flex justify-between">
              <div class="flex">
                <template v-for="(i, ind) in item.input">
                  <div class="mx-1">
                    <span>{{ findItemById(craftMap.items, i.id).name }}</span>
